...
| `.github/workflows/validate-planner-contracts.yml` | `push`, `pull_request` | Planner shell tests, workflow contract tests, bootstrap-query-tools smoke, trusted stacks SHA verification | Repository contents on workflow/action/script changes plus `stacks` gitlink / `.gitmodules` updates | CI check results only |
