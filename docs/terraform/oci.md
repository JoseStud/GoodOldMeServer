...
- GlusterFS volume mounted at `/mnt/swarm-shared` (replica-2 between both workers)
...
|------|-------------|------|---------|:--------:|
| <a name="input_oci_compartment_ocid"></a> [oci\_compartment\_ocid](#input\_oci\_compartment\_ocid) | OCI compartment OCID (from Infisical) | `any` | n/a | yes |
| <a name="input_oci_image_ocid"></a> [oci\_image\_ocid](#input\_oci\_image\_ocid) | OCI image OCID for the worker instances | `string` | `"ocid1.image.oc1..."` | no |
| <a name="input_ssh_ca_public_key"></a> [ssh\_ca\_public\_key](#input\_ssh\_ca\_public\_key) | SSH CA public key injected into instances via cloud-init for certificate-based auth | `any` | n/a | yes |
