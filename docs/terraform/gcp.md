...
- **Three firewall rules:**
  ...
  - `allow_ssh` — TCP port 22 from configurable CIDR blocks (`ssh_allowed_cidrs`, defaults to all) with `ssh-access` target tag
...
| <a name="input_ssh_allowed_cidrs"></a> [ssh\_allowed\_cidrs](#input\_ssh\_allowed\_cidrs) | List of CIDR blocks allowed to SSH into the witness instance | `list(string)` | <pre>["0.0.0.0/0", "::/0"]</pre> | no |
