...
| `.github/workflows/orchestrator.yml` | Single orchestration entry point for all infra and ansible push paths. Push triggers: `terraform/**`, `stacks`, `.gitmodules`, `ansible/**`, `.ansible-lint`. The inline `compute-context` job classifies push/dispatch behavior, validates dispatch payloads, and computes optional ansible phase tags. Also accepts `stacks-redeploy-intent-v5` repository dispatch and `workflow_dispatch` (optional `ansible_only` boolean input to skip TFC apply). Fans into `preflight`, `infra`, `ansible`, and `portainer` reusable stage workflows under the `infra-orchestrator` concurrency group. |
