...
if [[ $# -ne 1 ]]; then
  echo "Usage: $0 <meta|iac>"
  exit 1
fi

mode="$1"
if [[ "${mode}" != "meta" && "${mode}" != "iac" ]]; then
...
if [[ "${mode}" == "meta" ]]; then
  ...
  require_field '.meta.changed_stacks | type == "string"' ".meta.changed_stacks"
  require_field '.meta.host_sync_stacks | type == "string"' ".meta.host_sync_stacks"
  require_field '.meta.config_stacks | type == "string"' ".meta.config_stacks"
  require_field '.meta.structural_change | type == "boolean"' ".meta.structural_change"
  require_field '.meta.reason | type == "string"' ".meta.reason"
  require_field '.meta.changed_paths | type == "string"' ".meta.changed_paths"
  ...
  emit_output "changed_stacks" "$(as_output_string '.meta.changed_stacks')"
  emit_output "host_sync_stacks" "$(as_output_string '.meta.host_sync_stacks')"
  emit_output "config_stacks" "$(as_output_string '.meta.config_stacks')"
  emit_output "structural_change" "$(as_output_bool '.meta.structural_change')"
  emit_output "reason" "$(as_output_string '.meta.reason')"
  emit_output "changed_paths" "$(as_output_string '.meta.changed_paths')"
  ...
  require_field '.iac | type == "object"' ".iac"
  require_field '.iac.infra_workspace_changed | type == "boolean"' ".iac.infra_workspace_changed"
  require_field '.iac.portainer_workspace_changed | type == "boolean"' ".iac.portainer_workspace_changed"
  require_field '.iac.ansible_changed | type == "boolean"' ".iac.ansible_changed"
  require_field '.iac.stacks_gitlink_changed | type == "boolean"' ".iac.stacks_gitlink_changed"
  require_field '.iac.stacks_sha | type == "string"' ".iac.stacks_sha"
  require_field '.iac.changed_tf_roots | type == "array"' ".iac.changed_tf_roots"
  require_field '.iac.tfc_workspace_matrix | type == "array"' ".iac.tfc_workspace_matrix"

  emit_output "infra_workspace_changed" "$(as_output_bool '.iac.infra_workspace_changed')"
  emit_output "portainer_workspace_changed" "$(as_output_bool '.iac.portainer_workspace_changed')"
  emit_output "ansible_changed" "$(as_output_bool '.iac.ansible_changed')"
  emit_output "stacks_gitlink_changed" "$(as_output_bool '.iac.stacks_gitlink_changed')"
  emit_output "stacks_sha" "$(as_output_string '.iac.stacks_sha')"
  emit_output "changed_tf_roots_json" "$(jq -c '.iac.changed_tf_roots' <<<"${PLAN_JSON}")"
  emit_output "tfc_workspace_matrix_json" "$(jq -c '.iac.tfc_workspace_matrix' <<<"${PLAN_JSON}")"
fi
